...
@snark 
def compute(pre_lfa_data, lfa_data, results, correct_hash):
    # Compute commitment
    ...
    some_distance = LinCombFxp(sum([i == 2 for i in pre_lfa_data]))
    no_distance = LinCombFxp(sum([i == 3 for i in pre_lfa_data]))
